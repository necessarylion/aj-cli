  ...
  program
    ...
    .action(async (port: string) => {
      ...
      if (isLinux) {
        ...
        const proc = Bun.spawn(["ss", "-tlnp", `sport = :${portNum}`], { stdout: "pipe", stderr: "pipe" });
        ...
        const proc = Bun.spawn(["lsof", "-i", `:${portNum}`, "-P", "-n"], { stdout: "pipe", stderr: "pipe" });
        const output = await new Response(proc.stdout).text();
        await proc.exited;
        ...
        const pids = [...new Set(rows.map((r) => r.split(/\s+/)[1]).filter((p): p is string => !!p))];
        for (const pid of pids) {
          console.log(chalk.green(`\nProcess details (PID ${pid}):`));
          const ps = Bun.spawn(["ps", "-p", pid, "-o", "pid,ppid,user,%cpu,%mem,started,command"], {
            stdout: "pipe",
            stderr: "pipe",
          });
